...
exports.searchRestaurants = (0, https_1.onRequest)({ cors: true, timeoutSeconds: 30 }, async (req, res) => {
    ...
    if (queryValue.length < 2 || country.length < 2) {
        res.status(200).json({ suggestions: [] });
        return;
    }
    try {
        const suggestions = await (0, photon_search_1.searchRestaurantsViaPhoton)(queryValue, country, 10);
        res.status(200).json({ suggestions });
    }
    catch (error) {
...
exports.aggregateRestaurantStats = (0, firestore_1.onDocumentWritten)('opinions/{opinionId}', async (event) => {
